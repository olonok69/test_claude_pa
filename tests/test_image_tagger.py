...
# Mark all tests in this file as tagger tests
pytestmark = pytest.mark.tagger
...
if platform.system() == "Linux" and IS_TEST == "1" and LOCAL_ENV == "1":
    from image.image_tagger.endpoint_image_tagger import app
...
def test_endpoint_image_tagger():
    """
    Test endpoint image
    """

    with open(
        os.path.join(ROOT_DIR, "dummy_data", "test_nsfw", "images.json"), "rb"
    ) as f:
        docs = f.read()
...
def test_endpoint_error():
    ...
    with open(
        os.path.join(ROOT_DIR, "dummy_data", "test_nsfw", "nsfw_error.json"), "rb"
    ) as f:
        docs = f.read()
...
def test_endpoint_no_data():
    ...
    with open(
        os.path.join(ROOT_DIR, "dummy_data", "test_nsfw", "nsfw_no_data.json"), "rb"
    ) as f:
        docs = f.read()
...
def test_endpoint_wrong_extension():
    ...
    with open(
        os.path.join(ROOT_DIR, "dummy_data", "test_nsfw", "nsfw_wrong_extension.json"),
        "rb",
    ) as f:
        docs = f.read()
