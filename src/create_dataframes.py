...
def create_dataframe(create_dataframe: str, root_dir: str, config: dict):
    """
    Create Dataframes
    Args:
    ...
    if str(create_dataframe).lower() == "yes":
        ...
        logging.info(
            "Length of the registration data: {}".format(len(registration_data))
        )
        # Merge the data
        merged_data, merged_data_status, merged_data_list_key = data_merger(
            registration_data, demographic_data_badge, year=last_year
        )
