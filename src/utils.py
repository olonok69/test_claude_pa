...
def concatenate_qa_demografic_data(
    ...
    for i, badge_ID in enumerate(list_badge_id_events):
        ...
        for row in df_d:
            ...
            if question == "your main areas of interest":
        ...
        for key in qa[badge_ID]:
            qa[badge_ID][key] = qa[badge_ID][key][3:]

        pre_classification = (
            "Enought_data" if len(list_key) >= 6 else "Not_enought_data"
        )
...
def include_scan_Data_demo_data(demo_data, list_badgesid_with_seminars, merged_df):
    """
...
def create_dataframes(
    ...
    demo_data_final = include_scan_Data_demo_data(
        demo_data, list_badgesid_with_seminars, merged_df
    )
