...
6. **Neo4j visitor relationships (step 8)**
   ...
   - Matching is identity-first (`id_both_years = forename_surname_email`, lowercase), with badge as fallback.
   - Identity fallback is not limited to a single node, so attendance can attach across multiple campaign visitor nodes when applicable.
   - `_create_assisted_session_this_year_run_relationships` projects run-scoped attendance edges `assisted_session_this_year_run` from attendance + delivery lineage (`CampaignDelivery` -> `RecommendationRun`). This supports visitors mailed in multiple runs (e.g., `personal_agendas` + `engagement`) and preserves both attributions for downstream first/last-touch logic.
