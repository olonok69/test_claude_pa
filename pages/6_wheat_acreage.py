    ...
    if 'acreage_data_loaded' not in st.session_state:
        ...
        if acreage_data and projection_metadata:
            ...
            st.session_state.acreage_data = {
                "WORLD": {
                    "2021/2022": 220.8, "2021/2022_pct": None, "2021/2022_yield": 3.53,
                    "2022/2023": 224.79, "2022/2023_change": 3.99, "2022/2023_yield": 3.58,
                    "2023/2024": 228.18, "2023/2024_change": 3.39, "2023/2024_yield": 3.48,
                    "2024/2025": 226.5, "2024/2025_change": -1.68, "2024/2025_yield": 3.52
                }
            }
...
# Main content area
tab1, tab2, tab3, tab4, tab5 = st.tabs(["📈 Data Overview", "✏️ Edit Projections", "📊 Visualizations", "🌱 Yield Analysis", "💾 Data Export"])
...
with tab1:
    ...
    st.markdown(f"**Projection Dates**: {st.session_state.acreage_projection_metadata['last_updated']} | {st.session_state.acreage_projection_metadata['next_update']}")
    ...
    **Acreage (Area Harvested)** represents the total land area from which wheat is harvested.
    ...
    with col1:
        world_acreage = st.session_state.acreage_data.get('WORLD', {}).get('2024/2025', 0)
        st.metric("Global Acreage 2024/2025", f"{world_acreage:.1f} Mha")
    
    with col2:
        world_change = st.session_state.acreage_data.get('WORLD', {}).get('2024/2025_change', 0)
        st.metric("Change from 2023/2024", f"{world_change:+.1f} Mha")
    
    with col3:
        # Average yield
        world_yield = st.session_state.acreage_data.get('WORLD', {}).get('2024/2025_yield', 0)
        st.metric("Global Avg Yield", f"{world_yield:.2f} t/ha")
    
    with col4:
        # Top country by acreage (excluding WORLD and Others)
        countries_only = {k: v for k, v in st.session_state.acreage_data.items() 
                         if k not in ["WORLD", "Others"]}
        if countries_only:
            top_country = max(countries_only.items(), key=lambda x: x[1]['2024/2025'])
            st.metric("Largest Area", f"{top_country[0]}: {top_country[1]['2024/2025']:.1f} Mha")
...
with tab2:
    ...
    with st.form("acreage_projection_form"):
        ...
        for country in st.session_state.acreage_data.keys():
            ...
            current_change = st.session_state.acreage_data[country].get("2024/2025_change", 0)
            current_yield = st.session_state.acreage_data[country].get("2024/2025_yield", 0)
            ...
            with col4:
                ...
                auto_change = updated_values[country] - st.session_state.acreage_data[country]["2023/2024"]
                if auto_change > 0:
        ...
        if st.form_submit_button("Update Acreage Projections", type="primary"):
            ...
            for country, value in updated_values.items():
                ...
                st.session_state.acreage_data[country]["2024/2025_change"] = updated_changes[country]
                st.session_state.acreage_data[country]["2024/2025_yield"] = updated_yields[country]
                
                # Save to database
                if db:
                    db.update_acreage_value(country, "2024/2025", value, updated_changes[country], 
...
with tab3:
    ...
    countries_to_plot = st.multiselect(
        ...
        options=[c for c in st.session_state.acreage_data.keys() if c not in ["Others"]],
        default=["WORLD", "India", "Russia", "China", "European Union 27 (FR, DE)", "United States"]
    )
    ...
    countries_only = {k: v for k, v in st.session_state.acreage_data.items() 
                     if k not in ["WORLD", "Others"]}
    top_countries = sorted(countries_only.items(), key=lambda x: x[1]['2024/2025'], reverse=True)[:10]
    ...
    # Change analysis
    create_change_visualization(st.session_state.acreage_data, "Acreage", exclude=["WORLD", "Others"])
...
with tab4:
    ...
    **Yield** (tonnes per hectare) is a key productivity indicator that combines with acreage to determine production.
    ...
    fig_yield = px.bar(df_yield, x="Country", y="Yield",
    ...
    world_yield = st.session_state.acreage_data.get('WORLD', {}).get('2024/2025_yield', 3.52)
    fig_yield.add_hline(y=world_yield, line_dash="dash", line_color="red", 
    ...
    fig_scatter = px.scatter(df_yield, x="Acreage", y="Yield", 
                            text="Country", size="Acreage",
                            title="Wheat Acreage vs Yield (2024/2025)",
                            labels={"Acreage": "Area Harvested (Mha)", 
                                   "Yield": "Yield (t/ha)"})
    ...
    # Add quadrant labels
    fig_scatter.add_annotation(x=5, y=5.5, text="High Yield<br>Small Area", showarrow=False)
    fig_scatter.add_annotation(x=25, y=5.5, text="High Yield<br>Large Area", showarrow=False)
    fig_scatter.add_annotation(x=5, y=1.5, text="Low Yield<br>Small Area", showarrow=False)
    fig_scatter.add_annotation(x=25, y=1.5, text="Low Yield<br>Large Area", showarrow=False)
...
with tab5:
    ...
    with col1:
        ...
        export_data = {
            ...
            "data_source": "database" if st.session_state.acreage_data_loaded else "local"
        }
            ...
            try:
                ...
                if st.button("Import Acreage Data"):
                    if "wheat_acreage_data" in uploaded_data:
                        st.session_state.acreage_data = uploaded_data["wheat_acreage_data"]
                    
                    if "metadata" in uploaded_data:
                        st.session_state.acreage_projection_metadata = uploaded_data["metadata"]
...
# Footer
st.markdown("---")
status_text = "🗄️ Database Connected" if st.session_state.acreage_data_loaded else "💾 Local Data Mode"
st.markdown(
    f"""
