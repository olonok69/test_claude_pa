    ...
    if 'import_data_loaded' not in st.session_state:
        ...
        if import_data and projection_metadata:
            ...
            st.session_state.import_data = {
                "Egypt": {
                    "2021/2022": 12.0, "2021/2022_pct": None,
                    "2022/2023": 12.9, "2022/2023_change": 0.9,
                    "2023/2024": 12.8, "2023/2024_change": -0.1,
                    "2024/2025": 12.3, "2024/2025_change": -0.5
                },
                "Indonesia": {
                    "2021/2022": 10.5, "2021/2022_pct": None,
                    "2022/2023": 9.6, "2022/2023_change": -0.9,
                    "2023/2024": 13.1, "2023/2024_change": 3.5,
                    "2024/2025": 10.9, "2024/2025_change": -2.2
                }
            }
...
with tab1:
    ...
    st.markdown(f"**Projection Dates**: {st.session_state.import_projection_metadata['last_updated']} | {st.session_state.import_projection_metadata['next_update']}")
    ...
    with col1:
        total_imports = st.session_state.import_data.get('TOTAL MAJOR IMPORTERS', {}).get('2024/2025', 0)
        st.metric("Total Major Imports 2024/2025", f"{total_imports:.1f} Mt")
    
    with col2:
        total_change = st.session_state.import_data.get('TOTAL MAJOR IMPORTERS', {}).get('2024/2025_change', 0)
        st.metric("Change from 2023/2024", f"{total_change:+.1f} Mt")
    
    with col3:
        # Find top importer (excluding TOTAL)
        countries_only = {k: v for k, v in st.session_state.import_data.items() if k != "TOTAL MAJOR IMPORTERS"}
        if countries_only:
...
with tab2:
    ...
    with st.form("import_projection_form"):
        ...
        for country in st.session_state.import_data.keys():
            ...
            current_change = st.session_state.import_data[country].get("2024/2025_change", 0)
            ...
            with col3:
                ...
                auto_change = updated_values[country] - st.session_state.import_data[country]["2023/2024"]
                if auto_change > 0:
        ...
        if st.form_submit_button("Update Import Projections", type="primary"):
            ...
            for country, value in updated_values.items():
                ...
                st.session_state.import_data[country]["2024/2025_change"] = updated_changes[country]
                
                # Save to database
                if db:
                    db.update_import_value(country, "2024/2025", value, updated_changes[country])
            
            # Calculate and update total
            total_imports = sum(updated_values.values())
            total_change = sum(updated_changes.values())
            
            st.session_state.import_data["TOTAL MAJOR IMPORTERS"]["2024/2025"] = total_imports
            st.session_state.import_data["TOTAL MAJOR IMPORTERS"]["2024/2025_change"] = total_change
            
            if db:
                db.update_import_value("TOTAL MAJOR IMPORTERS", "2024/2025", total_imports, total_change)
...
with tab3:
    ...
    countries_to_plot = st.multiselect(
        "Select countries to display:",
        options=[c for c in st.session_state.import_data.keys() if c != "TOTAL MAJOR IMPORTERS"],
        default=["Egypt", "Indonesia", "EU-27", "Turkey", "China", "Philippines"]
    )
    ...
    if countries_to_plot:
        ...
        if st.checkbox("Show Total Major Importers"):
            total_values = [st.session_state.import_data["TOTAL MAJOR IMPORTERS"][year] for year in years]
            fig.add_trace(go.Scatter(
                x=years,
                y=total_values,
                mode='lines+markers',
    ...
    countries_only = {k: v for k, v in st.session_state.import_data.items() if k != "TOTAL MAJOR IMPORTERS"}
    top_importers = sorted(countries_only.items(), key=lambda x: x[1]['2024/2025'], reverse=True)[:10]
    ...
    # Change analysis
    create_change_visualization(st.session_state.import_data, "Imports", exclude=["TOTAL MAJOR IMPORTERS"])
...
with tab4:
    ...
    with col1:
        ...
        export_data = {
            ...
            "data_source": "database" if st.session_state.import_data_loaded else "local"
        }
            ...
            try:
                ...
                if st.button("Import Import Data"):
                    if "wheat_import_data" in uploaded_data:
                        st.session_state.import_data = uploaded_data["wheat_import_data"]
                    
                    if "metadata" in uploaded_data:
                        st.session_state.import_projection_metadata = uploaded_data["metadata"]
...
# Footer
st.markdown("---")
status_text = "🗄️ Database Connected" if st.session_state.import_data_loaded else "💾 Local Data Mode"
st.markdown(
    f"""
    <div style='text-align: center; color: #666; font-size: 0.8em;'>
    📥 Wheat Imports Dashboard | {status_text} | Major Importers Analysis
    </div>
    """,
