...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Deep Agents TODO Workflow for Pre-Show/Post-Show Reports")
    ...
    parser.add_argument(
        ...
        dest="report_type",
        default="pre_show",
        choices=["pre_show", "post_show"],
        help="Report type to generate",
    )
    parser.add_argument(
        "--pre-show-report",
        dest="pre_show_report",
        help="Path to a pre-show report for post-show comparison context",
    )
...
async def main() -> None:
    ...
    generator = ShowReportGenerator(
        ...
        pre_show_report_path=args.pre_show_report,
    ...
    try:
        ...
        print(
            f"🤖 Generating and executing complete {args.report_type.replace('_', '-')} "
            f"report for {args.event.upper()} {args.year}"
        )
        print("=" * 60)
