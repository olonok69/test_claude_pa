...
def resolve_config_path(raw_path: str, project_root: Path) -> Path:
    ...
    legacy_prefix = Path("deep_agents_reports")
    stripped = supplied
    if supplied.parts and supplied.parts[0] == legacy_prefix.as_posix():
        stripped = Path(*supplied.parts[1:])
    ...
    if supplied.is_absolute():
        ...
        candidate_paths.extend(
            [
                ...
                (project_root / "deep_agents_reports" / "config" / stripped),
                (project_root.parent / "config" / supplied),  # PA/config
                (project_root.parent / "config" / stripped),  # PA/config
            ]
        )
