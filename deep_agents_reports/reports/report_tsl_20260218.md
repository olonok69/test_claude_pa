# Tech Show London 2026 — Pre-Show Recommendation Report (Updated)

**Event:** Tech Show London (TSL) 2026
**Event Dates:** 4–5 March 2026
**Report Date:** 18 February 2026 (updated from 14 February 2026)
**Pipeline Run:** 14 February 2026, 14:44–15:30 UTC (~47 minutes)
**V2E Load:** 17 February 2026, 09:33–09:47 UTC
**Control Group Update:** 17 February 2026 (5% → 15%)
**Database:** Neo4j Production (neo4j-prod)
**Mode:** personal_agendas
**Configuration:** config/config_tsl.yaml
**Report Type:** Updated Run

---

## 1. Executive Summary

The recommendation pipeline processed **16,656 registered visitors** for TSL 2026, generating personalised session recommendations for every single registrant and complementing them with Visitor-to-Exhibitor (V2E) recommendations covering 99.7% of visitors. The system is operating at full coverage with strong data quality, well-calibrated similarity scoring, a properly implemented A/B control group at 15% allocation, and a newly loaded V2E exhibitor recommendation layer.

| Metric | Value | Assessment |
|--------|:-----:|:----------:|
| Total visitors | 16,656 | — |
| Visitors with session recommendations | 16,656 | ✅ 100% coverage |
| Total session recommendations | 166,489 | — |
| Average session recs per visitor | 10.0 | ✅ Target met |
| Visitors with V2E recommendations | 16,609 | ✅ 99.7% coverage |
| Total V2E recommendations | 83,045 | — |
| Exhibitors per visitor | 5 | ✅ Uniform |
| Returning visitors | 6,145 (36.9%) | Solid retention base |
| Control group | 2,496 (15.0%) | ✅ Correctly allocated |
...
| Overall Grade | **B+ (85/100)** | Good — minor cleanup needed |

**Key Strengths:** Full visitor coverage for both session and exhibitor recommendations, excellent data quality across all six similarity attributes (96.5% weighted fill rate), strong stream coverage (96.4%), well-distributed recommendations with no excessive concentration, properly implemented 15% control group with increased statistical power for A/B testing, and near-complete V2E exhibitor coverage (98.5% of exhibitors, 99.7% of visitors).

**Key Issues:** Nine sessions lack stream assignments (1 sponsored placeholder, 8 real content sessions with classifiable titles but no synopsis). Maximum session concentration is 39.3%, driven by DevOps/AI sessions dominating the top recommendation slots. Sixteen visitors received fewer than the target 10 recommendations due to overlap resolution, including one extreme outlier with only 1 recommendation. 1,394 visitors (8.4%) are missing sector data, the second-highest-weighted similarity attribute. The 15% control group shows a 5.9pp new/returning skew requiring segmented post-event analysis. V2E exhibitor concentration is high — the top 3.4% of exhibitors capture 42.4% of all V2E recommendations.
...
## 7. Visitor-to-Exhibitor (V2E) Recommendations
...
### 7.3 Exhibitor Distribution by Co-located Show
...
### 7.4 Exhibitor Recommendation Concentration
...
| Concentration | 7/10 | Top 3.4% exhibitors = 42.4% of recs; highly skewed |
...
## 8. Pipeline Configuration Summary

### 8.1 Similarity Weights
...
| Control group | ✅ Enabled (15%, seed 42) | Updated from 5% → 15% |
| Parallel processing | ✅ Enabled (12 workers) | — |
| Balance across days | ❌ Disabled | — |
| External recommendations (V2E) | ✅ Enabled | `v2e_recommendations_2.json`, loaded 17 Feb |
...
| V2E Coverage | 99.7% | 16,609/16,656 visitors with exhibitor recs |
| **Overall Data Quality** | **92.6%** | Average of all scores |

### 9.2 Strengths ✅

1. **100% session recommendation coverage** — every registered visitor receives personalised session suggestions
2. **99.7% V2E exhibitor coverage** — near-complete visitor-to-exhibitor recommendations across all co-located shows
3. **99.9% full recommendation sets** — only 16 visitors below the 10-session target (due to overlap resolution)
4. **Excellent attribute fill rates** — weighted average 96.5%, all attributes ≥89.2%
5. **Strong stream coverage** — 96.4% of sessions have stream assignments
6. **Well-calibrated scores** — 89.6% of recommendations in the 0.60–0.89 sweet spot, IQR of 0.111 shows meaningful differentiation
7. **Control group properly implemented** — 15.0% allocation with increased statistical power, balanced professional and sector demographics, score parity, IS_RECOMMENDED preserved for both groups
8. **Sample recommendations are coherent** — domain-focused, diverse, appropriate for visitor profiles across different seniority levels, sectors, and geographies
9. **Scalable pipeline** — processed 16,656 visitors in ~47 minutes with 12 parallel workers
10. **Session title filtering working** — no administrative/filler sessions (e.g., "Closing Remarks", "TBC Session") in the recommendation output
...
7. ⚠️ **Control group new/returning skew (5.9pp)** — the 15% control group over-represents returning visitors (41.9% vs 36.0%). Post-event analysis should segment by visitor type to account for this imbalance.

8. ⚠️ **V2E exhibitor concentration** — the top 13 exhibitors (3.4%) capture 42.4% of all V2E recommendations. While the maximum per-exhibitor concentration is lower than session concentration (18.1% vs 39.3%), the aggregate skew warrants review.
...
| 4 | **Investigate 47 visitors without V2E recommendations** | Data Team | Before next run | Close the 0.3% V2E coverage gap |
...
| # | Action | Owner | Timeline | Expected Impact |
|:-:|:-------|:------|:---------|:----------------|
| 5 | **Modify `backfill_missing_streams()`** to fall back to title-only classification when synopsis is empty | Engineering | Q2 2026 | Eliminate the recurring synopsis-dependent blind spot |
| 6 | **Monitor concentration trend** across incremental runs; if max exceeds 40%, consider implementing diversity cap | Data Science | Ongoing | Reduce concentration risk |
| 7 | **Consider increasing `similar_visitors_count`** from 3 to 5 for broader collaborative filtering | Data Science | Next event cycle | Improved recommendation quality for new visitors (63% of base) |
| 8 | **Capture sector data** from the 1,394 visitors with missing sector information via targeted email | Marketing | Before event | Increase second-highest-weighted attribute from 91.6% to ~100% |
| 9 | **Segment post-event A/B analysis** by new/returning status to control for the 5.9pp demographic skew in the 15% control group | Data Science | Post-event | Ensure accurate lift measurement |
| 10 | **Review V2E concentration** — investigate whether the top 13 exhibitors genuinely have the broadest relevance or if the matching algorithm over-weights certain attributes | Data Science | Post-event | Improve stand traffic distribution |

### Medium-Term — Next Pipeline Iteration (Quarter 1)

| # | Action | Owner | Timeline | Expected Impact |
|:-:|:-------|:------|:---------|:----------------|
| 11 | **Implement pattern-based title exclusions** (regex for "sponsored by", "presented by", "to be confirmed") | Engineering | Q2 2026 | Automated session inventory cleanup |
| 12 | **Explore FastRP graph embedding** for similarity computation | Data Science | Q2 2026 | Performance improvement and multi-hop reasoning |
| 13 | **Review stream taxonomy** — prune or consolidate the 16 unused streams | Data Team | Q2 2026 | Cleaner taxonomy, reduced noise |
| 14 | **Consider stratified control group allocation** — balance new/returning proportions within the random seed | Engineering | Q2 2026 | Eliminate demographic skew in future A/B tests |
...
|:---------|:-----:|:------|
| Session Coverage | 10/10 | 100% of visitors, 99.9% at full 10-rec target |
| V2E Coverage | 10/10 | 99.7% of visitors, 98.5% of exhibitors covered |
...
| Concentration | 8/10 | Session max 39.3%, V2E max 18.1% — acceptable but worth monitoring |
| Control Group | 9/10 | 15% allocation with increased statistical power; ⚠️ 5.9pp new/returning skew |
| **Overall** | **85/100 (B+)** | **Ready for event with minor cleanup** |

### Executive Wrap-Up

**Key Achievements:**
1. ✅ 100% session recommendation coverage across 16,656 visitors — every registrant receives 10 personalised session suggestions
2. ✅ 99.7% V2E exhibitor recommendation coverage — 16,609 visitors matched to 5 exhibitors each across 385 exhibitor stands
3. ✅ Excellent data quality: 96.5% weighted attribute fill rate across all six similarity dimensions
4. ✅ Well-calibrated similarity scoring: 89.6% of recommendations score 0.60–0.89, with meaningful differentiation (IQR 0.111)
5. ✅ Robust A/B testing infrastructure: 15% control group with increased statistical power (detectable lift reduced from ~3.5pp to ~2.0pp)
6. ✅ Scalable processing: 16,656 visitors processed in 47 minutes with 12 parallel workers
...
5. ⚠️ Control group new/returning skew (5.9pp) — requires segmented post-event analysis
6. ⚠️ V2E exhibitor concentration — top 3.4% of exhibitors capture 42.4% of recommendations

**Business Implications:**
1. **Attendee engagement:** 166,489 targeted session recommendations + 83,045 exhibitor recommendations provide a comprehensive driver for both session attendance and exhibitor footfall at a 251-session, 391-exhibitor event across 15 theatres
2. **Sponsor & exhibitor value:** DevOps and AI sessions dominate the most-recommended list, confirming strong attendee appetite for these tracks — sponsors in these areas should see strong footfall. V2E recommendations extend this value proposition directly to exhibitor stands, with 98.5% of exhibitors receiving at least one visitor recommendation
3. **A/B measurement readiness:** The 15% control group design enables rigorous post-event measurement of recommendation lift for both sessions and exhibitor visits, with improved statistical power to detect effects as small as 2.0pp. The new/returning skew should be controlled for in the analysis

**Bottom line:** The TSL 2026 recommendation system is operating well at scale. Full session coverage is achieved for all 16,656 visitors with near-complete V2E exhibitor coverage (99.7%), data quality is excellent, and the control group is correctly implemented at 15% for post-event impact measurement. The primary pre-event actions are assigning streams to 8 content sessions, investigating the 47 visitors missing V2E recommendations, and re-running the pipeline — estimated at 2–3 hours of work plus one pipeline execution. With 14 days until the event (4–5 March), there is ample runway for incremental refinements. The system is ready for deployment.

---

*Report updated 18 February 2026 from Neo4j Production database.*
*Session pipeline run: 14 February 2026 14:44–15:30 UTC | V2E load: 17 February 2026 09:33–09:47 UTC.*
*Configuration: config/config_tsl.yaml | Mode: personal_agendas | Control group: 15% (seed 42)*
