...
async def process_text(request: Request, out: index_response):
    """
    ...
    try:
        ...
        mapper = None
        if isinstance(response.get("filter_detection"), int) or isinstance(
            response.get("filter_detection"), bool
        ):
            if isinstance(response.get("filter_detection"), int):
                filter_detection = response.get("filter_detection")
                filter_detection = True if filter_detection == 1 else False
            if isinstance(response.get("filter_detection"), bool):
                filter_detection = response.get("filter_detection")
        else:
        ...
        if isinstance(response.get("score"), str) or isinstance(
            ...
            if (
                ...
                score = (
                    score
                    if score >= GLOBAL_DOCUMENT_MIN_SCORE
                    else GLOBAL_DOCUMENT_MIN_SCORE
                )
                ...
                score = GLOBAL_DOCUMENT_MIN_SCORE
        elif response.get("score") == None:
            score = GLOBAL_DOCUMENT_MIN_SCORE
        ...
        logging.info(
            f"Processing Text Engine {LANGUAGE_ENGINE} filter_detection: {str(filter_detection)}"
        )
...
async def process_tika(request: Request, out: index_response):
    ...
    try:
        ...
        logging.info("Start extract Docs method. Extracting data from Tika Documents")
        # FILTER_DETECTION
        ...
        if len(docs_with_languages) == 0:
            ...
            for d in documents_non_processed:
                for key, value in d.items():
                    logging.warning(f"Document {key} non processed, Reason {value}")
            out.data = []
            out.error = (
                "No Documents with Information or supported language in this batch"
            )
        ...
        if endpoint.mapper_version == "v1":
            logging.info("Start Analisys Process with Risk Model Version v1")
            ...
            logging.info("Start Analisys Process with Risk Model Version v2")
        ...
        logging.warning(
            f"Number of documents NO analyzed {len(documents_non_processed)}"
        )
        # logging reasons
        for d in documents_non_processed:
            for key, value in d.items():
        ...
        # delete objects
        del documents_non_processed
        del chunck
        del docs_with_languages
        del request
        del mapper
        return JSONResponse(content=json_compatible_item_data, status_code=200)
