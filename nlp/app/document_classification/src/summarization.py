...
def get_similarity_per_chunk(endpoint: FastAPI, docs: List[str], k: int = 5):
    """
    Get similarity per chunk
    Args:
        endpoint: FastAPI endpoint
        docs: List of documents
        k: int number of classes to return
    """
    results = {}
    # get top k classes per each chunk
    for i, doc in enumerate(docs):
        res = endpoint.db.similarity_search_with_score(doc.page_content, k=k)
        output = []
        for doc_result, score in res:
            output.append({doc_result.metadata.get("classifier"): score})
        results[i] = output
    # aggregate results from all chunks. If the same class appears in multiple chunks, average the scores
    final_output = {}
    ...
    # get top k classes
    k_keys_sorted = heapq.nlargest(k, final_output, key=final_output.get)
    res = []
    for key in k_keys_sorted:
        res.append({key: final_output[key]})
    return res


def get_classification_no_summarization(
    text: str, chunk_length: int, overlap: int, endpoint: FastAPI, k: int = 5
):
    """
    Get classification without summarization

    Args:
    text: str text to classify
    chunk_length: int size of chunk
    overlap: int overlap between chunks
    endpoint: FastAPI endpoint
    k: int number of classes to return
    """
    docs, num_documentos = split_text(
        text=text, chunk_size=chunk_length, chunk_overlap=overlap
    )
    res = get_similarity_per_chunk(endpoint, docs, k)
    return res, num_documentos
...
def get_summary(selected_docs, summarizer, selected_indices):
    """
    Get summary of selected docs
    Args:
    selected_docs: list of selected docs
...
def get_final_summary(summaries, endpoint):
    ...
    if len(summaries) > 4800:
        ...
        if length_tokens > 64 and length_tokens < 1024:
