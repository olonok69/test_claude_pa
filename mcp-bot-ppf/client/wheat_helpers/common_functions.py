...
from .database_helper import WheatProductionDB
import json
...
def create_status_indicators():
    ...
    with status_cols[2]:
        st.warning("**2023/2024**: estimate")
    with status_cols[3]:
        st.success("**2024/2025**: projection")

def create_projection_dates_sidebar(projection_metadata, last_updated_key, next_update_key):
...
def create_change_visualization(data, data_type="Production", exclude=None):
    ...
    for country, values in filtered_data.items():
        change_data.append({
            "Country": country,
            "2022/2023 Change": values.get("2022/2023_change", 0),
...
def validate_projection_data(value, historical_data):
    ...
    if deviation > 0.5:  # More than 50% deviation
        return False, f"Value deviates significantly from historical average ({avg_historical:.1f})"
