...
class WheatProductionDB:
    ...
    def get_all_yield_data(self) -> Dict:
        ...
        try:
            ...
            # Convert to the nested dictionary format
            data = {}
            for country, year, yield_val, change, change_pct, category, weather, status in rows:
                if country not in data:
                    data[country] = {}
    ...
    def update_yield_value(self, country: str, year: str, yield_val: float, change: float = None, 
                          change_pct: float = None, weather: str = None):
        ...
        try:
            ...
            old_data = cursor.fetchone()
            old_values = {"yield": old_data[0], "change": old_data[1], 
                         "change_pct": old_data[2], "weather": old_data[3]} if old_data else None
            ...
            cursor.execute('''
                ...
                WHERE country = ? AND year = ?
            ''', (yield_val, change, change_pct, category, weather, 
                  datetime.now().isoformat(), country, year))
            
            # Log the change
            if old_values:
                new_values = {"yield": yield_val, "change": change, 
                             "change_pct": change_pct, "weather": weather}
                self._log_audit(cursor, "wheat_yield", f"{country}_{year}", "UPDATE", 
                              old_values, new_values, "streamlit_user")
    ...
    def update_acreage_value(self, country: str, year: str, acreage: float, change: float = None, yield_val: float = None):
        ...
        try:
            ...
            old_data = cursor.fetchone()
            old_values = {"acreage": old_data[0], "change": old_data[1], "yield": old_data[2]} if old_data else None
            ...
            if old_values:
                ...
                self._log_audit(cursor, "wheat_acreage", f"{country}_{year}", "UPDATE", 
                              old_values, new_values, "streamlit_user")
    ...
    def update_su_ratio_value(self, country: str, year: str, su_ratio: float, change: float = None):
        ...
        try:
            ...
            old_data = cursor.fetchone()
            old_values = {"su_ratio": old_data[0], "change": old_data[1], "category": old_data[2]} if old_data else None
            ...
            # Log the change
            if old_values:
                new_values = {"su_ratio": su_ratio, "change": change, "category": category}
                self._log_audit(cursor, "wheat_su_ratio", f"{country}_{year}", "UPDATE", 
                              old_values, new_values, "streamlit_user")
    ...
    def update_stocks_value(self, country: str, year: str, stock: float, change: float = None, su_ratio: float = None):
        ...
        try:
            ...
            old_data = cursor.fetchone()
            old_values = {"stock": old_data[0], "change": old_data[1], "su_ratio": old_data[2]} if old_data else None
            ...
            if old_values:
                ...
                self._log_audit(cursor, "wheat_stocks", f"{country}_{year}", "UPDATE", 
                              old_values, new_values, "streamlit_user")
    ...
    def update_import_value(self, country: str, year: str, import_val: float, change: float = None):
        ...
        try:
            ...
            old_data = cursor.fetchone()
            old_values = {"import": old_data[0], "change": old_data[1]} if old_data else None
            ...
            if old_values:
                ...
                self._log_audit(cursor, "wheat_imports", f"{country}_{year}", "UPDATE", 
                              old_values, new_values, "streamlit_user")
    ...
    def update_export_value(self, country: str, year: str, export: float, change: float = None):
        ...
        try:
            ...
            old_data = cursor.fetchone()
            old_values = {"export": old_data[0], "change": old_data[1]} if old_data else None
            ...
            if old_values:
                ...
                self._log_audit(cursor, "wheat_exports", f"{country}_{year}", "UPDATE", 
                              old_values, new_values, "streamlit_user")
