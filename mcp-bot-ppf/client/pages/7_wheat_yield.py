    ...
    if "yield_data_loaded" not in st.session_state:
        ...
        if yield_data and projection_metadata:
            ...
            st.session_state.yield_data = {
                "WORLD": {
                    "2021/2022": 3.53,
                    "2021/2022_change": None,
                    "2021/2022_pct": None,
                    "2021/2022_category": "Medium",
                    "2021/2022_weather": "Normal",
                    "2022/2023": 3.58,
                    "2022/2023_change": 0.05,
                    "2022/2023_pct": 1.3,
                    "2022/2023_category": "Medium",
                    "2022/2023_weather": "Favorable",
                    "2023/2024": 3.48,
                    "2023/2024_change": -0.10,
                    "2023/2024_pct": -2.6,
                    "2023/2024_category": "Medium",
                    "2023/2024_weather": "Mixed",
                    "2024/2025": 3.52,
                    "2024/2025_change": 0.04,
                    "2024/2025_pct": 1.1,
                    "2024/2025_category": "Medium",
                    "2024/2025_weather": "Normal",
                }
            }
...
with tab1:
    ...
    st.markdown(
        f"**Projection Dates**: {st.session_state.yield_projection_metadata['last_updated']} | {st.session_state.yield_projection_metadata['next_update']}"
    )
    ...
    for country, data in st.session_state.yield_data.items():
        ...
        row = {
            ...
            "Change": (
                f"{data.get('2022/2023_change', 0):+.2f}"
                if data.get("2022/2023_change")
                else "-"
            ),
            "2023/2024": f"{data['2023/2024']:.2f}",
            "Change ": (
                f"{data.get('2023/2024_change', 0):+.2f}"
                if data.get("2023/2024_change")
                else "-"
            ),
            "2024/2025": f"{data['2024/2025']:.2f}",
            "Change  ": (
                f"{data.get('2024/2025_change', 0):+.2f}"
                if data.get("2024/2025_change")
                else "-"
            ),
    ...
    styled_df = (
        ...
        .set_properties(**{"text-align": "left"}, subset=["Country/Region", "Weather"])
    )
    ...
    with col2:
        ...
        countries_only = {
            k: v for k, v in st.session_state.yield_data.items() if k != "WORLD"
        }
        if countries_only:
            max_country = max(countries_only.items(), key=lambda x: x[1]["2024/2025"])
            st.metric(
                "Highest Yield", f"{max_country[0]}: {max_country[1]['2024/2025']:.2f}"
            )
    ...
    with col3:
        ...
        if countries_only:
            ...
            st.metric(
                "Lowest Yield", f"{min_country[0]}: {min_country[1]['2024/2025']:.2f}"
            )
...
with tab2:
    ...
    with st.form("yield_projection_form"):
        ...
        for country in st.session_state.yield_data.keys():
            ...
            current_change = st.session_state.yield_data[country].get(
                "2024/2025_change", 0
            )
            current_weather = st.session_state.yield_data[country].get(
                "2024/2025_weather", "Normal"
            )
            ...
            with col3:
                ...
                updated_weather[country] = st.selectbox(
                    ...
                    index=(
                        weather_options.index(current_weather)
                        if current_weather in weather_options
                        else 2
                    ),
        ...
        if st.form_submit_button("Update Yield Projections", type="primary"):
            ...
            for country, value in updated_values.items():
                ...
                pct_change = (
                    (updated_changes[country] / prev_value * 100)
                    if prev_value > 0
                    else 0
                )
                ...
                st.session_state.yield_data[country]["2024/2025_change"] = (
                    updated_changes[country]
                )
                st.session_state.yield_data[country]["2024/2025_pct"] = pct_change
                st.session_state.yield_data[country]["2024/2025_weather"] = (
                    updated_weather[country]
                )
...
with tab3:
    ...
    if countries_to_plot:
        ...
        if "WORLD" not in countries_to_plot:
            world_values = [
                st.session_state.yield_data["WORLD"][year] for year in years
            ]
            fig.add_trace(
                go.Scatter(
                    x=years,
...
with tab5:
    ...
    weather_summary = (
        df_weather.groupby("Weather")
        .agg({"Country": "count", "Yield": "mean", "Change": "mean"})
        .round(2)
    )
            ...
            try:
                ...
                if st.button("Import Yield Data"):
                    ...
                    if "metadata" in uploaded_data:
                        st.session_state.yield_projection_metadata = uploaded_data[
                            "metadata"
                        ]
