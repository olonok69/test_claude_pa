    ...
    if "su_ratio_data_loaded" not in st.session_state:
        ...
        if su_data and projection_metadata:
            ...
            st.session_state.su_ratio_data = {
                "WORLD": {
                    "2021/2022": 35.0,
                    "2021/2022_category": "Comfortable",
                    "2022/2023": 35.7,
                    "2022/2023_change": 0.7,
                    "2022/2023_category": "Comfortable",
                    "2023/2024": 33.7,
                    "2023/2024_change": -2.0,
                    "2023/2024_category": "Comfortable",
                    "2024/2025": 33.2,
                    "2024/2025_change": -0.5,
                    "2024/2025_category": "Comfortable",
                },
                "China": {
                    "2021/2022": 93.7,
                    "2021/2022_category": "Strategic Reserve",
                    "2022/2023": 98.2,
                    "2022/2023_change": 4.5,
                    "2022/2023_category": "Strategic Reserve",
                    "2023/2024": 93.5,
                    "2023/2024_change": -4.7,
                    "2023/2024_category": "Strategic Reserve",
                    "2024/2025": 97.3,
                    "2024/2025_change": 3.8,
                    "2024/2025_category": "Strategic Reserve",
                },
            }
...
with tab1:
    ...
    st.markdown(
        f"**Projection Dates**: {st.session_state.su_ratio_projection_metadata['last_updated']} | {st.session_state.su_ratio_projection_metadata['next_update']}"
    )
    ...
    with col1:
        ...
        world_change = st.session_state.su_ratio_data.get("WORLD", {}).get(
            "2024/2025_change", 0
        )
    ...
    with col3:
        # Highest S/U ratio
        max_country = max(
            st.session_state.su_ratio_data.items(),
            key=lambda x: x[1].get("2024/2025", 0),
        )
        st.metric(
            "Highest S/U Ratio", f"{max_country[0]}: {max_country[1]['2024/2025']:.1f}%"
        )

    with col4:
        # Lowest S/U ratio (excluding WORLD)
        countries_only = {
            k: v for k, v in st.session_state.su_ratio_data.items() if k != "WORLD"
        }
        min_country = min(
            countries_only.items(), key=lambda x: x[1].get("2024/2025", 100)
        )
        st.metric(
            "Lowest S/U Ratio", f"{min_country[0]}: {min_country[1]['2024/2025']:.1f}%"
        )
...
with tab2:
    ...
    with st.form("su_ratio_projection_form"):
        ...
        for country in st.session_state.su_ratio_data.keys():
            ...
            current_change = st.session_state.su_ratio_data[country].get(
                "2024/2025_change", 0
            )
        ...
        if st.form_submit_button("Update S/U Ratio Projections", type="primary"):
            ...
            for country, value in updated_values.items():
                ...
                st.session_state.su_ratio_data[country]["2024/2025_change"] = (
                    updated_changes[country]
                )
                st.session_state.su_ratio_data[country]["2024/2025_category"] = (
                    get_category(value)
                )
...
with tab3:
    ...
    fig_pie.update_layout(
        title="Distribution of Countries by S/U Ratio Category", height=400
    )
    ...
    for region_name, countries in regions.items():
        ...
        if region_data:
            ...
            with col2:
                ...
                max_country = df_region[df_region["2024/2025 S/U Ratio"] == max_su][
                    "Country"
                ].values[0]
                st.metric("Highest", f"{max_country}: {max_su:.1f}%")
            with col3:
                min_su = df_region["2024/2025 S/U Ratio"].min()
                min_country = df_region[df_region["2024/2025 S/U Ratio"] == min_su][
                    "Country"
                ].values[0]
            ...
            try:
                ...
                if st.button("Import S/U Ratio Data"):
                    if "wheat_su_ratio_data" in uploaded_data:
                        st.session_state.su_ratio_data = uploaded_data[
                            "wheat_su_ratio_data"
                        ]

                    if "metadata" in uploaded_data:
                        st.session_state.su_ratio_projection_metadata = uploaded_data[
                            "metadata"
                        ]
