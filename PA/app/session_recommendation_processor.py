...
def json_to_dataframe(json_file_path: str) -> pd.DataFrame:
    ...
    for rec in recommendations:
        ...
        filtered_recs = rec.get(
            "filtered_recommendations", []
        )  # List of dictionaries or strings
        rec_metadata = rec.get(
            "metadata", {}
...
class SessionRecommendationProcessor:
    ...
    def process(self, create_only_new=True):
        ...
        try:
            ...
            self.logger.info(f"Loaded {len(visitor_data)} visitors")
            ...
            # NEW: Convert JSON to DataFrame and save as CSV
            try:
                # Convert JSON to DataFrame
                self.logger.info(f"Converting JSON to DataFrame")
                df = json_to_dataframe(output_file)

                # Create CSV filename based on the JSON filename
                csv_filename = (
                    os.path.splitext(os.path.basename(output_file))[0] + ".csv"
                )
                csv_path = os.path.join(
                    self.output_dir, "recommendations", csv_filename
                )
                ...
                self.logger.error(
                    f"Error converting JSON to CSV: {str(e)}", exc_info=True
                )
                self.statistics["error_details"].append(
                    f"CSV conversion error: {str(e)}"
                )
