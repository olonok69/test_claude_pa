...
from src.utils import concatenate_showref_badgeid
...
# output model
root_filename = "gpt-4o-mini-2_2025-03-08_15-03-40"
filename = os.path.join(ROOT_DIR, "csm_data", "classification", f"{root_filename}.json")
# demo data with exclude
path_demodata_output_full = os.path.join(
    ROOT_DIR, "csm_data", output_data_folder, demographic_data_with_badge_json_full
)
# registration data before remove exclude columns
path_output_regdata_full = os.path.join(
    ROOT_DIR, "csm_data", output_csv_folder, registration_2025_full
)
# registration data RAW after transform from json
path_output_regdata = os.path.join(
    ROOT_DIR, "csm_data", output_csv_folder, registration_this_year
)
...
for ele, i in zip(regdata, range(len(regdata))):
    ...
    try:
        ...
        email, company, jobtitle, tel, mobile, postcode, country, forename, surname = (
            None,
            None,
            None,
            None,
...
filename = os.path.join(ROOT_DIR, "csm_data", "classification", f"{root_filename}.csv")
