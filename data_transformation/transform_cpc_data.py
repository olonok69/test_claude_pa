...
DATA_DIR = PROJECT_ROOT / "data" / "cpcn"
...
def transform_flat_demographics(records: Sequence[Dict[str, Any]], template_keys: Sequence[str], show_ref: str, source_filename: str) -> List[Dict[str, Any]]:
    ...
    for idx, item in enumerate(records, start=1):
        ...
        badge_id = get_mapped_value(item, DEMO_FLAT_MAP, "badge_id")
        question_id = safe_int(get_mapped_value(item, DEMO_FLAT_MAP, "question_id"))
        answer_id = safe_int(get_mapped_value(item, DEMO_FLAT_MAP, "answer_id"))
        record["id"] = make_demographic_id(badge_id, question_id, answer_id, show_ref)
        record["badge_id"] = badge_id
        record["event_id"] = get_mapped_value(item, DEMO_FLAT_MAP, "event_id") or event_id
        record["show_ref"] = (get_mapped_value(item, DEMO_FLAT_MAP, "show_ref") or show_ref or "UNKNOWN").upper()
        record["question_type"] = get_mapped_value(item, DEMO_FLAT_MAP, "question_type")
        record["question_id"] = question_id
        record["question_text"] = get_mapped_value(item, DEMO_FLAT_MAP, "question_text")
        record["answer_id"] = answer_id
        record["answer_text"] = get_mapped_value(item, DEMO_FLAT_MAP, "answer_text")
        record["is_positive"] = bool_to_flag(get_mapped_value(item, DEMO_FLAT_MAP, "is_positive"))
        record["response_text"] = get_mapped_value(item, DEMO_FLAT_MAP, "response_text")
...
def transform_nested_demographics(records: Sequence[Dict[str, Any]], template_keys: Sequence[str], show_ref: str, source_filename: str) -> List[Dict[str, Any]]:
    ...
    for base in records:
        ...
        source_value = get_mapped_value(base, DEMO_NESTED_BASE_MAP, "source")
        inferred_show = infer_show_ref(source_value, show_ref)
...
def transform_registration(records: Sequence[Dict[str, Any]], template_keys: Sequence[str], show_ref: str, source_filename: str) -> List[Dict[str, Any]]:
    ...
    for idx, item in enumerate(records, start=1):
        ...
        record["event_id"] = get_mapped_value(item, REGISTRATION_MAP, "event_id") or event_id
...
def main(include_legacy: bool = False) -> None:
    ...
    if cpcn25_demo_raw and isinstance(cpcn25_demo_raw[0], dict) and get_mapped_value(cpcn25_demo_raw[0], DEMO_NESTED_BASE_MAP, "question_responses") is not None:
        demo_records.extend(
