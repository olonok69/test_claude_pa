...
def pil2PIX32(im, leptonica, ffi):
    """Convert PIL to leptonica PIX."""
    # At the moment we handle everything as RGBA image, except mode L images in grey scale, only 1 channel
    # bug 6646  Image jog grey scale mode L. depth has to be 8 and can not be convertwd to RGBA or RGB
    ...
    if im.mode == "L":
        ...
        logging.info(f"Image mode L. width: {width}, height: {height}, depth: {depth}")
    elif im.mode == "RGBA":
        depth = 32
        width, height = im.size
        data = im.tobytes("raw")
        ...
        leptonica.pixSetData(pixs, ffi.from_buffer("l_uint32[]", data))
        logging.info(
            f"Image mode RGBA. width: {width}, height: {height}, depth: {depth}"
        )
    elif im.mode != "RGBA":
        im = im.convert("RGBA")
        depth = 32
        ...
        leptonica.pixSetData(pixs, ffi.from_buffer("l_uint32[]", data))
        logging.info(
            f"Image mode CONVERTED TO RGBA. width: {width}, height: {height}, depth: {depth}"
        )

    try:
        resolutionX = int(im.info["resolution"][0])
        resolutionY = int(im.info["resolution"][1])
        leptonica.pixSetResolution(pixs, resolutionX, resolutionY)
    except KeyError:
        logging.warn("Image info doesn't contains resolution key. Trying DPI")
    try:
        resolutionX = int(im.info["dpi"][0])
        resolutionY = int(im.info["dpi"][1])
        leptonica.pixSetResolution(pixs, resolutionX, resolutionY)
    except KeyError:
        logging.warn("Image info doesn't contains dpi key.")

    return leptonica.pixEndianByteSwapNew(pixs)


def setup_path_library_names(ffi):
    """
    Setup path and libraries
    """
    dll_dir = ""
    ...
    if os.path.exists(tess_libname):
        ...
        logging.warn(f"'{tess_libname}' does not exists!")
    tesseract_version = ffi.string(tesseract.TessVersion())
    logging.info(f"Tesseract-ocr version:  {tesseract_version.decode('utf-8')}")
    ...
    logging.info(f"Leptonica version:  {leptonica_version.decode('utf-8')}")

    return tesseract, leptonica


def initialize_tesseract_api(api, tesseract, tessdata):
...
def initialize_tesseract_api_set_image(api, pix, ffi, tesseract):
    # Parameters fro API initialization
