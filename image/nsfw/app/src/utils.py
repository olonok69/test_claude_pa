...
def analyse_video(data, model, processor, device, threshold, use_onnx: str):
    """
...
def analyse_image(data, model, threshold, processor, device, use_onnx: str):
    """
    ...
    try:
        ...
        image_preds = {}
        preds = predict_image_pytorch(
            temp_filename, processor, model, device, use_onnx=use_onnx
        )
...
def nsfw_analisys_documents_from_request(
    document, model, threshold, processor, device, use_onnx: str
):
    """
    Process images depending of their type
    params:
        document: dictionary containing metadata and data of a document
        model model (ONNX Vit or Transformers flavor ViT)
        threshold : score to decide if nsfw or not
        processor : ViT tokenizer
        device: cuda or cpu
        use_onnx: if use onnx or ViT Transformer
    return:
    label classification and data dictionary
    ...
    if ext in video_file_names:
        ...
        utf8_text, document = analyse_video(
            document, model, processor, device, threshold, use_onnx=use_onnx
        )
    elif ext in image_file_names:
        logging.info(f"Inference image File with extension {ext}")
        utf8_text, document = analyse_image(
            document, model, threshold, processor, device, use_onnx=use_onnx
        )
...
def process_request(
    ...
    for data in list_docs:
        ...
        utf8_text, data = nsfw_analisys_documents_from_request(
            data, model, threshold, processor, device, use_onnx=use_onnx
        )
        ...
        jobs[new_task.uid].status = f"Analized successfully {len(processed_docs)} files"
        logging.info(f"Analized successfully {len(processed_docs)} files")
