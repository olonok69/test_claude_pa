...
from nsfw.inference import load_model
...
# Load model USE_ONNX = YES ve load the onnx model if not our custom VIT model
try:
    if USE_ONNX == "YES":
        model_path = os.path.join(MODELS_PATH, "onnx", "vit_nsfw.onnx")
        logging.info(f"Loaded Model {model_path}. Using ONNX Model")
    else:
        model_path = os.path.join(MODELS_PATH, "nsfw_pytorch")
        logging.info(f"Loaded Model {model_path}. Using Pytorch Model")
    # load model
...
#     ):
#         # ONly run in local or in local docker
#         uvicorn.run(
