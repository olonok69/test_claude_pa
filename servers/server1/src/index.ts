...
const MAX_RESPONSE_LENGTH = 100000; // 100KB limit for individual responses
const MAX_CONTENT_PREVIEW = 5000; // Preview length for content in listings
    ...
    switch (name) {
      case 'firecrawl_scrape': {
        ...
        try {
          ...
          const finalContent = truncateResponse(fullContent, MAX_RESPONSE_LENGTH);
      ...
      case 'firecrawl_map': {
        ...
        // Limit the number of links returned to avoid overwhelming the response
        const maxLinks = 500;
        const links = response.links.slice(0, maxLinks);
        const truncatedMessage = response.links.length > maxLinks 
          ? `\n\n[Showing first ${maxLinks} of ${response.links.length} total links]`
          : '';
        
        return {
          content: [
            { type: 'text', text: trimResponseText(links.join('\n') + truncatedMessage) },
          ],
          isError: false,
        };
      }
      ...
      case 'firecrawl_check_crawl_status': {
        ...
        const formattedResults = response.data.length > 0 
          ? formatResultsWithLimit(response.data, MAX_CONTENT_PREVIEW)
          : '';
...
${formattedResults ? '\nResults:\n' + formattedResults : ''}`;
      ...
      case 'firecrawl_search': {
        ...
        try {
          ...
          // Format the results with content truncation
          const results = response.data
            .map(
              (result) => {
                const content = result.markdown 
                  ? truncateResponse(result.markdown, MAX_CONTENT_PREVIEW)
                  : '';
...
${content ? `\nContent:\n${content}` : ''}`;
      ...
      case 'firecrawl_extract': {
        ...
        try {
          ...
          const truncatedData = truncateResponse(extractedData, MAX_RESPONSE_LENGTH);
          
          const result = {
            content: [
              {
                type: 'text',
                text: trimResponseText(truncatedData),
              },
            ],
            isError: false,
          };
      ...
      case 'firecrawl_deep_research': {
        ...
        try {
          ...
          // Truncate the final analysis if needed
          const finalAnalysis = truncateResponse(
            response.data.finalAnalysis,
            MAX_RESPONSE_LENGTH
          );
          ...
          return {
            content: [
              {
                type: 'text',
                text: trimResponseText(finalAnalysis),
              },
            ],
            isError: false,
          };
      ...
      case 'firecrawl_generate_llmstxt': {
        ...
        try {
          ...
          // Truncate if needed
          const truncatedResult = truncateResponse(resultText, MAX_RESPONSE_LENGTH);

          return {
            content: [{ type: 'text', text: trimResponseText(truncatedResult) }],
            isError: false,
          };
        } catch (error) {
          const errorMessage =
...
// Helper function to format results with content limit
function formatResultsWithLimit(data: FirecrawlDocument[], contentLimit: number): string {
  return data
    .map((doc) => {
      const content = doc.markdown || doc.html || doc.rawHtml || 'No content';
      const truncatedContent = truncateResponse(content, contentLimit);
...
function formatResults(data: FirecrawlDocument[]): string {
  return formatResultsWithLimit(data, 100);
}

// Utility function to truncate responses
...
async function runSSELocalServer() {
  ...
  // Add JSON parsing middleware with increased limit
