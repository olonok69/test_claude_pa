# servers/server3/perplexity_sse_server.py - Fixed version
import os
import json
import logging
import aiohttp
...
# Create MCP server and transport - FIXED PORT TO 8003
mcp = FastMCP("Perplexity MCP Server", host="0.0.0.0", port=8003)
...
def validate_environment():
    ...
    logger.info(
        "Available Perplexity models (set with PERPLEXITY_MODEL environment variable):"
    )
...
async def call_perplexity_api(
    query: str, recency: str = "month", **kwargs
) -> Dict[str, Any]:
...
async def perplexity_advanced_search(
    ...
    logger.info(
        f"Advanced Perplexity search - Query: '{query}', Model: '{model}', "
    ...
    try:
        ...
        data = await call_perplexity_api(
            query, recency, model=model, max_tokens=max_tokens, temperature=temperature
        )
...
async def health_check(request):
    ...
    try:
        ...
        if (
            health_check_cache["last_check"] > 0
            and current_time - health_check_cache["last_check"]
            < health_check_cache["ttl"]
        ):
        ...
        result = {
            ...
            "server_info": {
                ...
                "uptime_seconds": (
                    int(current_time - start_time) if "start_time" in globals() else 0
                ),
...
async def handle_sse(request):
    """Handle SSE connections for MCP with improved error handling and connection tracking."""
    connection_id = id(request)
    active_connections.add(connection_id)
    logger.info(
        f"New SSE connection {connection_id}. Active: {len(active_connections)}"
    )

    try:
        # Ensure request._send is available and callable
        if not hasattr(request, "_send") or request._send is None:
            logger.error(f"Connection {connection_id}: request._send is None")
            return
        ...
        # Don't re-raise to avoid the NoneType callable error
        pass
    finally:
        # Always remove from active connections
        active_connections.discard(connection_id)
        logger.info(
            f"Connection {connection_id} cleaned up. Active: {len(active_connections)}"
        )
...
# Build the complete Starlette app with all routes and error handling
app = Starlette(
    routes=[
    ...
    exception_handlers={500: handle_500_error},
)
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.info(
            f"Starting Perplexity MCP Server v{__version__} with SSE transport and caching..."
        )
        logger.info("Perplexity MCP Server running on http://0.0.0.0:8003")
        ...
        logger.info("Enhanced error handling and connection tracking enabled")
