...
def handle_authentication_state_change(prev_auth_status, current_auth_status, 
    ...
    # User switched (logged out and logged in as different user)
        ...
        # Logout previous user (this will disconnect MCP servers)
        ...
        st.sidebar.info(f"🔄 Switched to {current_username} - MCP servers disconnected")
    
    # User logged out
    elif not current_auth_status and prev_auth_status and prev_username:
        logging.info(f"🚪 User {prev_username} logged out")
        on_user_logout(prev_username)  # This will now disconnect MCP servers
        ...
        st.sidebar.success("🚪 Logged out - MCP servers disconnected")
