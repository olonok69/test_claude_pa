...
from ui_components.system_prompt_tab import create_system_prompt_tab
...
def main():
    ...
    # MODIFIED: Create tabs based on permissions and MCP connection status
    mcp_connected = st.session_state.get("agent") is not None
    ...
    # Add System Prompt tab only if MCP is connected
    if mcp_connected:
        base_tabs.append("🎯 System Prompt")
    ...
    # Assign tabs to variables for easy reference
    tab1 = tabs[0]  # Chat
    tab2 = tabs[1]  # Configuration
    tab3 = tabs[2]  # Connections
    tab4 = tabs[3]  # Tools
    
    # Assign conditional tabs
    system_prompt_tab = None
    user_management_tab = None
    
    tab_index = 4  # Start after the base tabs
    
    if mcp_connected:
        system_prompt_tab = tabs[tab_index]
        tab_index += 1
    
    if is_admin and USER_MANAGEMENT_AVAILABLE:
        user_management_tab = tabs[tab_index]
    
    # Show why tabs are not available (if applicable)
    if not mcp_connected:
        st.sidebar.info("🎯 System Prompt: Connect to MCP servers first")
    ...
    with st.sidebar:
        ...
        # ADD: Show MCP logout warning if connected
        show_mcp_logout_warning()
        show_connection_status_on_logout()
    ...
    # ADDED: System Prompt Tab (only if MCP is connected)
    if system_prompt_tab is not None:
        with system_prompt_tab:
            check_authentication()
            create_system_prompt_tab()
...
def show_debug_info():
    ...
    if st.checkbox("🐛 Show Debug Info"):
        ...
        if st.checkbox("Show System Prompt Info"):
            st.markdown("### System Prompt Information")
            system_prompt = st.session_state.get('system_prompt')
            if system_prompt:
                st.write(f"**System Prompt Length:** {len(system_prompt)} characters")
                st.write(f"**System Prompt Words:** {len(system_prompt.split())} words")
                st.write(f"**Is Custom:** {st.session_state.get('system_prompt_is_custom', False)}")
                
                # Show first 200 characters
                st.text_area(
                    "System Prompt Preview",
                    value=system_prompt[:200] + "..." if len(system_prompt) > 200 else system_prompt,
                    height=100,
                    disabled=True
                )
            else:
                st.write("**System Prompt:** Not set")
...
def create_user_session_info():
    ...
    with st.expander("👤 User Session Details", expanded=False):
        ...
        system_prompt = st.session_state.get('system_prompt')
        if system_prompt:
            st.write(f"System Prompt: {len(system_prompt)} chars")
        else:
            st.write("System Prompt: Not set")
