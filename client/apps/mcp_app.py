...
def main():
    ...
    st.title("🔍 Google Search MCP Client - AI Chat Interface")
    ...
    with tab1:
        ...
        # Main chat interface
        st.header("💬 Chat with Google Search AI Agent")
        st.markdown("Ask questions to search the web and extract content from web pages.")
        ...
        # Chat input
        user_text = st.chat_input("Ask a question or search the web using Google Search tools")
    ...
    **Possible solutions:**
    ...
    3. **Check MCP server connection** - try disconnecting and reconnecting in the Connections tab
    ...
    **What happened:** The agent tried too many steps without reaching a conclusion, which usually means it got stuck in a loop or encountered tool execution issues."""
                        ...
                        # Fall back to regular stream response if agent not available
                            ...
                            st.warning("⚠️ You are not connected to the Google Search MCP server! Please connect in the Connections tab.")
