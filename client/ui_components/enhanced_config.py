    ...
    try:
        ...
        if provider_name == "OpenAI":
            ...
            try:
                ...
                return (
                    False,
                    "Mistral AI library not installed. Run: pip install mistralai",
                )
...
def create_quick_setup_section(saved_configs):
    ...
    with col2:
        ...
        status, configured_vars, missing_vars = get_provider_status(
    ...
    with col1:
        selected_model = st.selectbox(
            ...
            index=(
                provider_config["models"].index(provider_config["default_model"])
    ...
    with col1:
        if st.button(
            "💾 Save Configuration", use_container_width=True, key="quick_save"
        ):
            config_data = {
    ...
    with col2:
        if st.button(
            "🔄 Set as Active", use_container_width=True, key="quick_activate"
        ):
            st.session_state["params"]["model_id"] = selected_provider
            st.session_state["params"]["selected_model"] = selected_model
...
def create_advanced_setup_section(saved_configs):
    ...
    show_custom_provider = st.checkbox(
        "➕ Add Custom Provider", key="show_custom_provider"
    )
    if show_custom_provider:
        ...
        with col1:
            custom_name = st.text_input(
                "Provider Name", placeholder="My Custom Provider", key="custom_name"
            )
            custom_base_url = st.text_input(
                "Base URL", placeholder="https://api.example.com/v1", key="custom_url"
            )
            custom_api_key_name = st.text_input(
        ...
        with col2:
            custom_model = st.text_input(
                "Model Name", placeholder="custom-model-v1", key="custom_model"
            )
...
GOOGLE_API_KEY=AIza...""",
    ...
    show_model_params = st.checkbox(
        "⚙️ Model Parameters", key="show_model_params", value=True
    )
...
def create_connection_management_section(saved_configs):
    ...
    with col3:
        if st.button(
            "🧪 Test Connection", use_container_width=True, key="test_connection"
        ):
    ...
    with col1:
        if st.button(
            "🔄 Set as Active Provider", use_container_width=True, key="set_active"
        ):
            if test_provider and test_model:
    ...
    with col2:
        if st.button(
            "💾 Save Current Config", use_container_width=True, key="save_current"
        ):
            if test_provider and test_model:
                config_data = {
                    ...
                    "environment_vars": {
                        var: bool(os.getenv(var))
                        for var in EXTENDED_MODEL_OPTIONS[test_provider]["required_env"]
                        + EXTENDED_MODEL_OPTIONS[test_provider]["optional_env"]
                    },
                }
    ...
    if saved_configs:
        ...
        for provider_name, config_data in saved_configs.items():
            ...
            show_config = st.checkbox(
                f"📋 Show {provider_name} Configuration",
                key=f"show_config_{provider_name}",
            )
            if show_config:
                col1, col2, col3 = st.columns(3)
                ...
                with col1:
                    ...
                    st.write(
                        f"**Configured:** {config_data.get('configured_at', 'Unknown')[:10]}"
                    )

                with col2:
                    if st.button(f"Load {provider_name}", key=f"load_{provider_name}"):
                        st.session_state["params"]["model_id"] = provider_name
                        st.session_state["params"]["selected_model"] = config_data.get(
                            "model"
                        )
    ...
    show_env_manager = st.checkbox(
        "🌍 Environment Variable Manager", key="show_env_manager"
    )
    if show_env_manager:
        ...
        for provider_config in EXTENDED_MODEL_OPTIONS.values():
            for var in (
                provider_config["required_env"] + provider_config["optional_env"]
            ):
    ...
    with col1:
        if st.button(
            "📤 Export Configuration", use_container_width=True, key="export_config"
        ):
            config_export = {
        ...
        if uploaded_config:
            try:
                ...
                if "providers" in imported_config:
                    success, message = save_model_configuration(
                    ...
                    if success:
                        ...
                        if (
                            "current_provider" in imported_config
                            and imported_config["current_provider"]
                        ):
                            st.session_state["params"]["model_id"] = imported_config[
                                "current_provider"
                            ]
                            if "current_model" in imported_config:
                                st.session_state["params"]["selected_model"] = (
                                    imported_config["current_model"]
                                )
                        st.rerun()
                    else:
    ...
    show_package_helper = st.checkbox(
        "📦 Package Installation Helper", key="show_package_helper"
    )
    if show_package_helper:
        ...
        all_packages = (
            "pip install openai anthropic google-generativeai cohere mistralai requests"
        )
...
def add_provider_comparison():
    ...
    show_comparison = st.checkbox(
        "📊 Show Provider Comparison", key="show_provider_comparison"
    )
    if show_comparison:
