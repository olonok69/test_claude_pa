...
def create_advanced_setup_section(saved_configs):
    ...
    with st.expander("🌍 Bulk Environment Setup", expanded=False):
...
def create_connection_management_section(saved_configs):
    ...
    if saved_configs:
        ...
        for provider_name, config_data in saved_configs.items():
            with st.expander(f"📋 {provider_name} Configuration", expanded=False):
                col1, col2, col3 = st.columns(3)
    ...
    with st.expander("🌍 Environment Variable Manager", expanded=False):
    ...
    # Installation helper
    with st.expander("📦 Package Installation Helper", expanded=False):
