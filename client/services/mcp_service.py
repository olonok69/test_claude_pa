...
async def read_resource_from_client(client: MultiServerMCPClient, uri: str) -> str:
    ...
    try:
        ...
        resource_response = await client._send_request("resources/read", {"uri": uri})
...
async def use_prompt(
    client: MultiServerMCPClient, prompt_name: str, arguments: Dict = None
):
    ...
    try:
        ...
        prompt_response = await client._send_request(
            "prompts/get", {"name": prompt_name, "arguments": arguments}
        )
        ...
        for key, value in config.items():
            if (
                ...
                result[key] = os.getenv(
                    env_var, value
                )  # Use original value if env var not found
...
def connect_to_mcp_servers():
    ...
    for name, config in prepared_servers.items():
        ...
        if transport_type == "stdio":
            st.write(
                f"- {name}: stdio (command: {config.get('command')}, args: {config.get('args')})"
            )
        else:
    ...
    try:
        ...
        st.session_state.tools = run_async(
            get_tools_from_client(st.session_state.client)
        )

        # Get prompts and resources from MCP servers
        try:
            st.session_state.prompts = run_async(
                get_prompts_from_client(st.session_state.client)
            )
        except:
            st.session_state.prompts = []

        try:
            st.session_state.resources = run_async(
                get_resources_from_client(st.session_state.client)
            )
        except:
        ...
        st.success(
            f"Successfully connected to {len(prepared_servers)} MCP servers with {tool_count} tools, {prompt_count} prompts, and {resource_count} resources"
        )
        ...
        for tool in st.session_state.tools:
            ...
            if any(
                ...
                firecrawl_tools.append(tool.name)
        ...
        if firecrawl_tools:
            st.info(f"🔥 Firecrawl tools: {', '.join(firecrawl_tools)}")
        if google_tools:
        ...
        if st.session_state.prompts:
            prompt_names = [
                prompt.get("name", "unnamed") for prompt in st.session_state.prompts
            ]
