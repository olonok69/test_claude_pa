...
def init_session():
    # Initialize event loop first if needed
...
def _append_message_to_session(msg: dict) -> None:
    ...
    if st.session_state["messages"] and len(st.session_state["messages"]) > 0:
        ...
        if (
            last_message.get("role") == msg.get("role")
            and last_message.get("content") == msg.get("content")
            and last_message.get("tool") == msg.get("tool")
        ):
    ...
    if st.session_state.get("current_chat_id"):
        ...
        for msg in messages:
            if msg["role"] == "user" and "content" in msg:
                ...
                and msg["content"]
                and "tool" not in msg
            ):  # Only regular assistant messages, not tool messages
