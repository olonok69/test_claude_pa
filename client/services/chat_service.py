...
class PerformanceMonitor:
    ...
    def __init__(self, max_history: int = 1000):
        ...
        # FIXED: Initialize _lock FIRST before any other operations
        self._lock = threading.Lock()
        # Now safely initialize other attributes
        self._initialize_metrics()
    
    def _initialize_metrics(self):
        """Initialize all metrics safely within lock."""
        with self._lock:
    ...
    def get_tool_performance_analysis(self) -> dict:
        """ADDED: Get detailed tool performance analysis - MISSING METHOD FIXED."""
        self._ensure_lock()
        ...
        with self._lock:
            ...
            return {
                ...
                'summary': {
                    ...
                    'unique_tools': len(tool_analysis),
    ...
    def get_performance_trends(self, window_size: int = 10) -> dict:
        """ADDED: Get performance trends - MISSING METHOD FIXED."""
        self._ensure_lock()
        ...
        with self._lock:
            ...
            # Short-term trend (last window_size messages)
            recent_times = times[-window_size:]
            older_times = times[-window_size*2:-window_size] if len(times) >= window_size*2 else times[:-window_size]
            
            trends = {'status': 'active'}
...
def create_safe_performance_monitor() -> PerformanceMonitor:
    ...
    try:
        ...
        if not hasattr(monitor, '_lock') or monitor._lock is None:
            monitor._lock = threading.Lock()
            monitor._initialize_metrics()
        return monitor
