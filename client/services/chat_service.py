...
def on_user_logout(username: str):
    """Handle user logout - clear user-specific session data and disconnect from MCP servers."""
    ...
    # ADD: Disconnect from MCP servers on logout
    try:
        ...
        # Continue with logout even if MCP disconnect fails
    
    logging.info(f"🚪 User {username} logged out and MCP servers disconnected")