...
# Page configuration
st.set_page_config(
    page_title="Corn World Demand Dashboard",
    page_icon="🌍",
    layout="wide",
...
def initialize_session_state():
    ...
    if "corn_demand_data_loaded" not in st.session_state:
        demand_data, metadata, current_config = load_world_demand_data()

        if demand_data and metadata:
            st.session_state.corn_demand_data = demand_data
            st.session_state.corn_demand_metadata = metadata
            st.session_state.corn_demand_current_config = current_config
            st.session_state.corn_demand_data_loaded = True
        else:
            # Fallback to empty data
            st.session_state.corn_demand_data = {}
            st.session_state.corn_demand_metadata = {}
            st.session_state.corn_demand_current_config = {
            ...
            st.session_state.corn_demand_data_loaded = False
...
# Title and header
st.title("🌍 Corn World Demand Dashboard")
st.markdown("### Global Corn Consumption Analysis by Category")

# Database status indicator
if st.session_state.corn_demand_data_loaded:
...
with tab1:
    ...
    if "corn_demand_current_config" in st.session_state:
        st.markdown("### Status Information")
        status_cols = st.columns(4)

        display_years = st.session_state.corn_demand_current_config["display_years"]
        year_status = st.session_state.corn_demand_current_config["year_status"]
    ...
    st.info(
        """
    **World Demand** represents global corn consumption across different categories:
    - **Feed**: Animal feed consumption (largest category ~60%)
    - **Food**: Human consumption
    - **Industrial**: Industrial uses (ethanol, starch, sweeteners, etc.)
    ...
    for category in DEMAND_CATEGORIES:
        if category not in st.session_state.corn_demand_data:
            continue
        ...
        for i, year in enumerate(display_years):
            if year in st.session_state.corn_demand_data[category]:
                value = st.session_state.corn_demand_data[category][year]
                row[year] = f"{value:.1f}"

                # Add percentage for first year if available
                if (
                    i == 0
                    and f"{year}_pct" in st.session_state.corn_demand_data[category]
                ):
                    pct = st.session_state.corn_demand_data[category][f"{year}_pct"]
                    row[year] += f" ({pct}%)"

                # Calculate change from previous year (except for first year)
                if i > 0:
                    prev_year = display_years[i - 1]
                    if prev_year in st.session_state.corn_demand_data[category]:
                        change = st.session_state.corn_demand_data[category].get(
                            f"{year}_change", 0
                        )
    ...
    with col1:
        if (
            "Total Consumption" in st.session_state.corn_demand_data
            and latest_year in st.session_state.corn_demand_data["Total Consumption"]
        ):
            total_demand = st.session_state.corn_demand_data["Total Consumption"][
                latest_year
            ]
    ...
    with col2:
        if (
            "Total Consumption" in st.session_state.corn_demand_data
            and latest_year in st.session_state.corn_demand_data["Total Consumption"]
            and prev_year in st.session_state.corn_demand_data["Total Consumption"]
        ):
            change = (
                st.session_state.corn_demand_data["Total Consumption"][latest_year]
                - st.session_state.corn_demand_data["Total Consumption"][prev_year]
            )
            st.metric("Change from Previous Year", f"{change:+.1f} Mt")

    with col3:
        if (
            "Feed" in st.session_state.corn_demand_data
            and latest_year in st.session_state.corn_demand_data["Feed"]
        ):
            feed_demand = st.session_state.corn_demand_data["Feed"][latest_year]
            st.metric("Feed Demand", f"{feed_demand:.1f} Mt")

    with col4:
        if (
            "Industrial" in st.session_state.corn_demand_data
            and latest_year in st.session_state.corn_demand_data["Industrial"]
        ):
            industrial_demand = st.session_state.corn_demand_data["Industrial"][
                latest_year
            ]
...
with tab2:
    ...
    with st.form("corn_demand_projection_form"):
        ...
        for category in editable_categories:
            if projection_year not in st.session_state.corn_demand_data.get(
                category, {}
            ):
                continue

            current_value = st.session_state.corn_demand_data[category][projection_year]
            ...
            for year in display_years[:-1]:
                if year in st.session_state.corn_demand_data[category]:
                    historical_values.append(
                        st.session_state.corn_demand_data[category][year]
                    )
            ...
            with col2:
                ...
                if estimate_year in st.session_state.corn_demand_data[category]:
                    calc_change = (
                        updated_values[category]
                        - st.session_state.corn_demand_data[category][estimate_year]
                    )
                    change_pct = (
                        (
                            calc_change
                        ...
                        * 100
                        if st.session_state.corn_demand_data[category][estimate_year]
                        > 0
                        else 0
                    )
        ...
        if st.form_submit_button("Update Demand Projections", type="primary"):
            ...
            for category, value in updated_values.items():
                ...
                st.session_state.corn_demand_data[category][projection_year] = value
                st.session_state.corn_demand_data[category][
            ...
            if estimate_year in st.session_state.corn_demand_data.get(
                "Total Consumption", {}
            ):
                total_change = (
                    total_consumption
                    - st.session_state.corn_demand_data["Total Consumption"][
                        estimate_year
                    ]
                )
                total_change_pct = (
                    ...
                    * 100
                    if st.session_state.corn_demand_data["Total Consumption"][
                        estimate_year
                    ]
                    > 0
                    else 0
            ...
            if "Total Consumption" not in st.session_state.corn_demand_data:
                st.session_state.corn_demand_data["Total Consumption"] = {}
...
with tab3:
    ...
    categories_to_plot = st.multiselect(
        "Select categories to display:",
        options=[
            cat for cat in DEMAND_CATEGORIES if cat in st.session_state.corn_demand_data
        ],
        default=["Feed", "Food", "Industrial", "Total Consumption"],
    )
    ...
    if categories_to_plot:
        ...
        for category in categories_to_plot:
            ...
            for year in display_years:
                if year in st.session_state.corn_demand_data[category]:
                    values.append(st.session_state.corn_demand_data[category][year])
        ...
        fig.update_layout(
            title="Corn Demand Trends by Category",
            xaxis_title="Year",
        ...
        if len(display_years) > 1:
            for i, year in enumerate(display_years):
                if (
                    st.session_state.corn_demand_current_config["year_status"].get(year)
                    == "projection"
                ):
                    fig.add_vline(
                        x=i - 0.5,
    ...
    for category in DEMAND_CATEGORIES:
        if (
            ...
            and category in st.session_state.corn_demand_data
        ):
            if latest_year in st.session_state.corn_demand_data[category]:
                composition_data.append(
                    {
                        "Category": category,
                        "Demand": st.session_state.corn_demand_data[category][
                            latest_year
                        ],
                    }
                )
    ...
    if composition_data:
        ...
        fig_pie = px.pie(
            ...
            values="Demand",
            names="Category",
            title=f"Corn Demand Composition - {latest_year}",
            hole=0.3,
    ...
    change_data = []
    for category in DEMAND_CATEGORIES:
        if category in st.session_state.corn_demand_data:
            row = {"Category": category}
            ...
            for i in range(1, len(display_years)):
                ...
                if (
                    year in st.session_state.corn_demand_data[category]
                    and prev_year in st.session_state.corn_demand_data[category]
                ):
                    change = (
                        st.session_state.corn_demand_data[category][year]
                        - st.session_state.corn_demand_data[category][prev_year]
                    )
...
with tab4:
        ...
        if category in st.session_state.corn_demand_data:
            ...
            for i in range(1, len(display_years)):
                ...
                if (
                    ...
                    prev_value = st.session_state.corn_demand_data[category][prev_year]
                    curr_value = st.session_state.corn_demand_data[category][year]
                    if prev_value > 0:
            ...
            if growth_rates:
                ...
                latest_value = st.session_state.corn_demand_data[category].get(
                    latest_year, 0
                )
        ...
        for _, row in df_growth.iterrows():
            ...
            fig_bubble.add_trace(
                go.Scatter(
                    ...
                    name=row["Category"],
                    marker=dict(
                        size=row["Latest Demand"] / 10,  # Scale bubble size
                        color=color,
                        opacity=0.6,
                    ),
    ...
    with col1:
        ...
        if "Feed" in st.session_state.corn_demand_data:
            feed_latest = st.session_state.corn_demand_data["Feed"].get(latest_year, 0)
            feed_pct = (
                feed_latest
                / st.session_state.corn_demand_data.get("Total Consumption", {}).get(
                    latest_year, 1
                )
            ) * 100
            ...
            st.info(
                "Feed is the largest demand category, driven by global livestock production and meat consumption trends."
            )

    with col2:
        st.markdown("### 🏭 Industrial Demand")
        if "Industrial" in st.session_state.corn_demand_data:
            industrial_latest = st.session_state.corn_demand_data["Industrial"].get(
                latest_year, 0
            )
            industrial_pct = (
                industrial_latest
            ...
            st.metric("Current Demand", f"{industrial_latest:.1f} Mt")
            st.metric("% of Total", f"{industrial_pct:.1f}%")
            st.info(
...
with tab5:
    ...
    with col1:
        ...
        # Prepare export data
        export_data = {
            "corn_world_demand_data": st.session_state.corn_demand_data,
            "metadata": st.session_state.corn_demand_metadata,
            "current_config": st.session_state.corn_demand_current_config,
            "export_timestamp": datetime.now().isoformat(),
            "data_source": (
                "database" if st.session_state.corn_demand_data_loaded else "local"
            ),
        ...
        st.download_button(
            ...
            file_name=f"corn_world_demand_data_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json",
            mime="application/json",
        )

        # CSV export
        df_export = pd.DataFrame(st.session_state.corn_demand_data).T
        csv_data = df_export.to_csv()
        st.download_button(
            label="📥 Download Demand Data as CSV",
            data=csv_data,
            file_name=f"corn_world_demand_data_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv",
            mime="text/csv",
        )
        ...
        if uploaded_file is not None:
            try:
                ...
                if st.button("Import Demand Data"):
                    if "corn_world_demand_data" in uploaded_data:
                        st.session_state.corn_demand_data = uploaded_data[
                            "corn_world_demand_data"
                        ]

                    if "metadata" in uploaded_data:
                        st.session_state.corn_demand_metadata = uploaded_data[
                            "metadata"
                        ]

                    if "current_config" in uploaded_data:
                        st.session_state.corn_demand_current_config = uploaded_data[
                            "current_config"
                        ]
...
st.markdown(
    ...
    🌍 Corn World Demand Dashboard | {status_text} | {user_info} | PPF Europe Analysis Platform
    </div>
    """,
    unsafe_allow_html=True,
)
