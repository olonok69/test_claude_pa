...
with tab1:
    ...
    st.markdown(f"**Projection Dates**: {st.session_state.production_projection_metadata['last_updated']} | {st.session_state.production_projection_metadata['next_update']}")
    ...
    with st.form("projection_form"):
        ...
        if st.form_submit_button("Update Projections", type="primary"):
            ...
            for country, value in updated_values.items():
                ...
                # Save to database
                if db:
                    db.update_production_value(country, "2024/2025", value, updated_changes[country])
...
with tab3:
    ...
    # Change analysis
    create_change_visualization(st.session_state.wheat_data, "Production", exclude=["WORLD"])
...
with tab4:
    ...
    with col1:
        ...
        export_data = {
            ...
            "data_source": "database" if st.session_state.production_data_loaded else "local"
        }
            ...
            try:
                ...
                if st.button("Import Data"):
                    ...
                    if "metadata" in uploaded_data:
                        st.session_state.production_projection_metadata = uploaded_data["metadata"]
...
# Footer
st.markdown("---")
status_text = "🗄️ Database Connected" if st.session_state.production_data_loaded else "💾 Local Data Mode"
st.markdown(
    f"""
    <div style='text-align: center; color: #666; font-size: 0.8em;'>
    🌾 Wheat Production Dashboard | {status_text}
    </div>
    """,
    unsafe_allow_html=True
)