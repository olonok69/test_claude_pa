    ...
    if 'stocks_data_loaded' not in st.session_state:
        ...
        if stocks_data and projection_metadata:
            ...
            st.session_state.stocks_data = {
                "WORLD": {
                    "2021/2022": 274.3, "2021/2022_pct": None, "2021/2022_s_u": 35.2,
                    "2022/2023": 283.9, "2022/2023_change": 9.6, "2022/2023_s_u": 35.3,
                    "2023/2024": 272.2, "2023/2024_change": -11.7, "2023/2024_s_u": 34.2,
                    "2024/2025": 267.0, "2024/2025_change": -5.2, "2024/2025_s_u": 33.5
                },
                "China": {
                    "2021/2022": 132.9, "2021/2022_pct": 48.5, "2021/2022_s_u": 95.0,
                    "2022/2023": 140.3, "2022/2023_change": 7.4, "2022/2023_s_u": 100.2,
                    "2023/2024": 140.1, "2023/2024_change": -0.2, "2023/2024_s_u": 102.6,
                    "2024/2025": 142.9, "2024/2025_change": 2.8, "2024/2025_s_u": 102.1
                }
            }
...
# Main content area
tab1, tab2, tab3, tab4, tab5 = st.tabs(["📈 Data Overview", "✏️ Edit Projections", "📊 Visualizations", "📐 Stock-to-Use Analysis", "💾 Data Export"])
...
with tab1:
    ...
    st.markdown(f"**Projection Dates**: {st.session_state.stocks_projection_metadata['last_updated']} | {st.session_state.stocks_projection_metadata['next_update']}")
    ...
    **Key Insights:**
    ...
    for country, data in st.session_state.stocks_data.items():
        row = {
            ...
            "S/U Ratio": f"{data.get('2024/2025_s_u', 0):.1f}%" if data.get('2024/2025_s_u') else "-"
        }
    ...
    with col2:
        world_change = st.session_state.stocks_data.get('WORLD', {}).get('2024/2025_change', 0)
        st.metric("Change from 2023/2024", f"{world_change:+.1f} Mt")
    
    with col3:
        world_su_ratio = st.session_state.stocks_data.get('WORLD', {}).get('2024/2025_s_u', 0)
...
with tab2:
    ...
    with st.form("stocks_projection_form"):
        ...
        for country in st.session_state.stocks_data.keys():
            ...
            current_change = st.session_state.stocks_data[country].get("2024/2025_change", 0)
            ...
            with col4:
                ...
                auto_change = updated_values[country] - st.session_state.stocks_data[country]["2023/2024"]
                if auto_change > 0:
        ...
        if st.form_submit_button("Update Stock Projections", type="primary"):
            ...
            for country, value in updated_values.items():
                ...
                st.session_state.stocks_data[country]["2024/2025_change"] = updated_changes[country]
                if country in updated_su_ratios:
                    st.session_state.stocks_data[country]["2024/2025_s_u"] = updated_su_ratios[country]
                
                # Save to database
                if db:
                    db.update_stocks_value(country, "2024/2025", value, updated_changes[country], 
...
with tab3:
    ...
    countries_only = {k: v for k, v in st.session_state.stocks_data.items() if k != "WORLD"}
    ...
    fig_pie.update_layout(
        title="Distribution of Global Wheat Stocks - 2024/2025 Projection",
        height=500
    )
    ...
    # Change analysis
    create_change_visualization(st.session_state.stocks_data, "Stocks", exclude=["WORLD", "Others"])
...
with tab4:
    ...
    **Stock-to-Use (S/U) Ratio** indicates the level of carryover stock as a percentage of total use.
    ...
    for country, data in st.session_state.stocks_data.items():
        if country != "Others" and data.get("2024/2025_s_u"):
            su_data.append({
                ...
                "Category": "Strategic Reserve" if data["2024/2025_s_u"] > 50 else 
                           "Comfortable" if data["2024/2025_s_u"] > 30 else 
                           "Adequate" if data["2024/2025_s_u"] > 20 else "Tight"
            })
    ...
    fig_su = px.bar(df_su, x="Country", y="S/U Ratio", color="Category",
                     color_discrete_map={
    ...
    # Add reference lines
    fig_su.add_hline(y=20, line_dash="dash", line_color="red", 
                     annotation_text="Tight Supplies", annotation_position="right")
    fig_su.add_hline(y=30, line_dash="dash", line_color="orange", 
                     annotation_text="Adequate", annotation_position="right")
    fig_su.add_hline(y=50, line_dash="dash", line_color="green", 
                     annotation_text="Strategic Reserve", annotation_position="right")
    ...
    countries_su = st.multiselect(
        ...
        options=[c for c in st.session_state.stocks_data.keys() 
    ...
    if countries_su:
        ...
        for country in countries_su:
            ...
            fig_su_trend.add_trace(go.Scatter(
                x=years,
                y=su_values,
                mode='lines+markers',
                name=country,
...
with tab5:
    ...
    with col1:
        ...
        export_data = {
            ...
            "data_source": "database" if st.session_state.stocks_data_loaded else "local"
        }
            ...
            try:
                ...
                if st.button("Import Stocks Data"):
                    if "wheat_stocks_data" in uploaded_data:
                        st.session_state.stocks_data = uploaded_data["wheat_stocks_data"]
                    
                    if "metadata" in uploaded_data:
                        st.session_state.stocks_projection_metadata = uploaded_data["metadata"]
...
# Footer
st.markdown("---")
status_text = "🗄️ Database Connected" if st.session_state.stocks_data_loaded else "💾 Local Data Mode"
st.markdown(
    f"""
    <div style='text-align: center; color: #666; font-size: 0.8em;'>
    🏢 Wheat Ending Stocks Dashboard | {status_text} | Strategic Reserves Analysis
    </div>
    """,
    unsafe_allow_html=True
)