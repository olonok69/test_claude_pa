...
class RegistrationProcessor:
    ...
    def find_demographics_for_returning_visitors(self) -> None:
        ...
        # Get badge IDs for returning visitors
        list_badgeid_24_25_bva = set(list(self.df_bva_returning["BadgeId"].unique()))
        list_badgeid_24_25_lva = set(list(self.df_lvs_returning["BadgeId"].unique()))

        # Filter demographic data for returning visitors
        df_demo_24_25_bva = self.df_bva_demo_last_year[
            self.df_bva_demo_last_year["BadgeId"].isin(list_badgeid_24_25_bva)
        ]
        df_demo_24_25_lva = self.df_lvs_demo[
            self.df_lvs_demo["BadgeId"].isin(list_badgeid_24_25_lva)
        ]
    ...
    def combine_demographic_with_registration(self):
        ...
        df_reg_24_25_bva_valid_columns = self.df_reg_24_wdemo_data_bva[
            past_valid_columns
        ].copy()
        df_reg_24_25_lva_valid_columns = self.df_reg_24_wdemo_data_lva[
            past_valid_columns
        ].copy()

        # only badges with demo data
        df_reg_25_valid_columns = df_reg_25_valid_columns[
            df_reg_25_valid_columns["BadgeId"].isin(self.demo_badge_id_25)
        ]
        df_reg_24_25_bva_valid_columns = df_reg_24_25_bva_valid_columns[
            df_reg_24_25_bva_valid_columns["BadgeId"].isin(self.demo_badge_id_24_bva)
        ]
        df_reg_24_25_lva_valid_columns = df_reg_24_25_lva_valid_columns[
            df_reg_24_25_lva_valid_columns["BadgeId"].isin(self.demo_badge_id_24_lva)
        ]
        ...
        self.logger.info(
            f"Process demographic data. Created demographic data for {len(demo_data_this)} this year, {len(demo_data_last_bva)} last year BVA, and {len(demo_data_last_lva)} last year LVS"
        )
