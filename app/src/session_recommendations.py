...
try:
    ...
    has_langchain = False
    logging.warning("LangChain not available. Will use rule-based filtering only.")
...
class StreamlitSessionRecommendationService:
        ...
        try:
            with GraphDatabase.driver(self.uri, auth=(self.username, self.password)) as driver:
                with driver.session() as session:
                    ...
                    RETURN v.BadgeId as badge_id, v.Email as email, v.Country as country, 
                           v.JobTitle as job_title, v.job_role as job_role
                    ORDER BY v.BadgeId
                    """
    ...
    def recommend_sessions_optimized(self, badge_id: str, min_score: float = 0.0, 
        ...
        Args:
            badge_id: Visitor's badge ID
            min_score: Minimum similarity score for recommendations
