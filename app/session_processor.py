...
class SessionProcessor:
    ...
    def __init__(self, config: Dict):
        ...
        # Initialize the use_cached_descriptions parameter from config
        ...
        # Dictionary for sponsor abbreviation mapping
        self.map_vets = {
        ...
        # Titles to filter out
        self.titles_to_remove = ["exhibitor showcase", "tbc session", "-"]
    ...
    def load_env_variables(self) -> None:
        ...
        try:
            ...
            if not status or "OPENAI_API_KEY" not in self.env_config:
                self.logger.warning(
                    "Failed to load environment variables or API key missing"
                )
        ...
        try:
            ...
            for title in self.titles_to_remove:
                self.session_this_filtered = self.session_this_filtered[
                    ~(
                        self.session_this_filtered["title"].str.contains(
                ...
                self.session_last_filtered_bva = self.session_last_filtered_bva[
                    ~(
                        self.session_last_filtered_bva["title"].str.contains(
                            title, case=False, na=False
                        )
                    )
                ]
                ...
                self.session_last_filtered_lva = self.session_last_filtered_lva[
                    ~(
                        self.session_last_filtered_lva["title"].str.contains(
                            title, case=False, na=False
                        )
                    )
                ]
            ...
            self.session_last_filtered_bva = (
                self.session_last_filtered_bva.drop_duplicates(
                    subset=["date", "key_text"]
                )
            )
            self.session_last_filtered_lva = (
                self.session_last_filtered_lva.drop_duplicates(
                    subset=["date", "key_text"]
                )
            )
    ...
    def setup_language_model(self) -> None:
        ...
        try:
            ...
            self.llm = ChatOpenAI(
                model=self.config.get("language_model", {}).get(
                    "model", "gpt-4.1-mini"
                ),
                openai_api_key=self.env_config["OPENAI_API_KEY"],
                temperature=self.config.get("language_model", {}).get(
                    "temperature", 0.5
                ),
