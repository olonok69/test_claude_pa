...
async def chat(profile, profile_id, position):  # Add position
    logging.info(f"Sending profile {profile_id} (position {position}) to LLM...")
    params = {
        "num_ctx": 16184,
        "temperature": 0.5,
        "top_p": 0.9,
        "top_k": 30,
        "num_thread": 100,  # Adjust as needed
    }
    ...
    try:
        ...
        logging.info(
            f"Received response for profile {profile_id} (position {position})"
        )
        return response.message.content
    except Exception as e:
        logging.error(
            f"Error during chat for profile {profile_id} (position {position}): {e}"
        )
        return json.dumps(
...
async def process_profiles(profiles_with_ids, batch_size: int = 10):
    output = []
    ...
    async def process_batch(batch):
        ...
        llm_tasks = [
            chat(profile, profile_id, position)
...
async def main():
    ...
    # list_profiles = list_profiles[:100]  # For testing

    profiles_with_ids = []
    for i, p in enumerate(list_profiles):  # Enumerate to get position
        ...
        if status == "Enought_data":
            ...
            profiles_with_ids.append((profile_template, p, status, i))  # Add position
        else:
            profiles_with_ids.append((profile, p, status, i))  # Add position

    output = await process_profiles(profiles_with_ids)

    timestamp_str = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    model_id_safe = model_id.replace(":", "_")  # Sanitize model ID for filename
